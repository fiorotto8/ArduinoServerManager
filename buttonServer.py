...
def setSource():
    ...
    parser.add_argument("-p", "--position", help='Position to set in Arduino', action='store', type=float, default=-1)
    parser.add_argument("-hole", "--hole", help='Hole index to set in Arduino', action='store', type=int, default=-1)
    ...
    selectedPosition = client.odb_get("/Configurations/StepperMotor/SelectedPosition")
    positions = np.array(client.odb_get("/Configurations/StepperMotor/Position"))

    if args.hole != -1:
        args.position = positions[args.hole]
        # print(args) DEBUG
    if args.hole == -1 and args.position==-1:
        args.hole     = selectedPosition
        args.position = positions[selectedPosition]
    
    try:
        if args.position not in positions:
            hole = -2
        else:
            hole = np.where(positions==args.position)[0][0]
        ...
        client.odb_set("/Configurations/SourceState/source_position", -1)
        ...
        if args.position ==0:
            ...
            if args.position==float(final_pos[0][-5:]):
                ...
                client.odb_set("/Configurations/SourceState/source_position", hole)
            else:
